...
if __name__ == '__main__':
    ...
    parser.add_argument("--mode", default='twohands', type=str, help='options: twohands, cb, twohands_obj1, twohands_obj2')
    parser.add_argument("--img_dir", default='../myimages/images', type=str)
    parser.add_argument("--twohands_dir", default='../myimages/pred_twohands', type=str)
    ...
    parser.add_argument("--vis_dir", default='../myimages/pred_twohands_vis', type=str)
    ...
    for img_file in glob.glob(args.img_dir + '/*'):
        ...
        if args.mode == 'twohands':
            ...
            assert twohands_vis.max() > 2
            imsave(os.path.join(args.vis_dir, fname + '.jpg'), twohands_vis.astype(np.uint8))

        elif args.mode == 'cb':
            cb = np.array(Image.open(os.path.join(args.cb_dir, fname + '.png')))
            cb_vis = visualize_cb(img, cb)
            assert cb_vis.max() > 2
            imsave(os.path.join(args.vis_dir, fname + '.jpg'), cb_vis.astype(np.uint8))       
            ...
            assert twohands_obj1_vis.max() > 2
            imsave(os.path.join(args.vis_dir, fname + '.jpg'), twohands_obj1_vis.astype(np.uint8))
            ...
            assert twohands_obj2_vis.max() > 2
            imsave(os.path.join(args.vis_dir, fname + '.jpg'), twohands_obj2_vis.astype(np.uint8))
